...
@export_range(0.00, 0.5, 0.01) var zoom_smooth := 0.18     # 丝滑程度（0=无过渡；小=更慢更顺；大=更快）

# 可选：进场自动设为 Current，避免无激活相机
@export var force_current_on_ready := true
...
func _ready() -> void:
	if force_current_on_ready:
		make_current()

	_col = clamp(start_col, 0, max(grid_cols - 1, 0))
	_row = clamp(start_row, 0, max(grid_rows - 1, 0))
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.pressed:
		const BTN_WHEEL_UP := 4      # 上滚：缩小
		const BTN_WHEEL_DOWN := 5    # 下滚：放大

		if event.button_index == BTN_WHEEL_DOWN:
			# 放大：以鼠标为锚点
			zl_begin_cursor_locked_zoom()
			_target_zoom *= Vector2(wheel_step_in, wheel_step_in)
		elif event.button_index == BTN_WHEEL_UP:
			# 缩小：以屏幕中心为锚点
			zl_cancel_cursor_locked_zoom()
			_target_zoom *= Vector2(wheel_step_out, wheel_step_out)
		else:
			return
...
# 取消鼠标锚定（用于缩小时回到以屏幕中心为锚点的默认行为）
func zl_cancel_cursor_locked_zoom() -> void:
	if has_meta("zl_lock"):
