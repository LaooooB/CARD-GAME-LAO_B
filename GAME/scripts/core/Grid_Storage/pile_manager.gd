...
func _rebound_to_pre_drag() -> void:
	...
	# 回弹过程中把 z 恢复，以免长时间遮挡
	var target := _pre_drag_global
	_rebound_tween = create_tween()
	_rebound_tween.set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
	_rebound_tween.tween_property(self, "global_position", target, 0.16)

	_rebound_tween.finished.connect(func ():
		# 回到原层级
		z_index = _orig_z
		...
		if _cards.size() > 0:
			...
			var anim := top.find_child("CardAnimation", true, false)
			if anim != null and anim.has_method("bump"):
				anim.call("bump")
